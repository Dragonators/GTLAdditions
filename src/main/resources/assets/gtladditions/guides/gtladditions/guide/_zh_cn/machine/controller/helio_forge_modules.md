...
Helio 系列是 <ItemLink id="gtladditions:forge_of_the_antichrist" /> 的工作模块。模块需要安装在伪神之煅炉周围的模块位置，并连接到一台已成型且正在工作的主机；未连接或主机未运行时，模块不会推进配方。
...
* 只有连接的 <ItemLink id="gtladditions:forge_of_the_antichrist" /> 已达到最大效率并正在运行时才会工作。
