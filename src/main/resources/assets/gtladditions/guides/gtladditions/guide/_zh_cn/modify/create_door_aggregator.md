---
navigation:
  title: 创造之门与创造聚合仪
  parent: modify/modify_index.md
  position: 2
categories:
  - 魔改
item_ids:
  - gtceu:door_of_create
  - gtceu:create_aggregation
  - ae2:annihilation_plane
  - ae2:formation_plane
  - minecraft:chain_command_block
  - minecraft:repeating_command_block
---

# 创造之门与创造聚合仪

<Column gap="15" fullWidth={true}>

<Row>
    <BlockImage id="gtceu:door_of_create" scale="4" />

    <BlockImage id="gtceu:create_aggregation" scale="4" />
</Row>

<Column gap="2" fullWidth={true}>

* 创造之门与创造聚合仪现在支持 <ItemLink id="gtladditions:thread_modifier_hatch" />。
* 最大并行数为 1 加线程修改仓提供的额外线程数。
* <ItemLink id="ae2:annihilation_plane" /> 可以收回创造聚合仪生成的 <ItemLink id="minecraft:chain_command_block" /> 与 <ItemLink id="minecraft:repeating_command_block" />。
* 需要用 <ItemLink id="ae2:formation_plane" /> 大量抛出物品时，见 [AE2 自动化](ae2_automation.md)。

</Column>

## 创造之门配方

<Row>
    <Recipe id="gtladditions:door_of_create/command_block" />

    <Recipe id="gtladditions:door_of_create/magmatter_block" />
</Row>

## 创造聚合仪配方

<Row>
    <Recipe id="gtladditions:create_aggregation/chain_command_block" />

    <Recipe id="gtladditions:create_aggregation/repeating_command_block" />
</Row>

</Column>